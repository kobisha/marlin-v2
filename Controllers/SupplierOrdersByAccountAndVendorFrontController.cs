...
namespace Marlin.sqlite.Controllers
{
    ...
    [ApiController]
    [Authorize]
    public class SupplierOrdersByAccountAndVendorFrontController : ControllerBase
    {
        private readonly DataContext _context;

        public SupplierOrdersByAccountAndVendorFrontController(DataContext context)
        {
            _context = context;
        }

        [HttpGet("{RetailID}/{VendorID}")]
        public IActionResult GetData(string RetailID, string VendorID)
        {
            try
            {
                var query = @"
...
where oh.""AccountID"" = @VendorID and oh.""ReceiverID"" = @AccountID
